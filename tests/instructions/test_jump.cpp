...
// Estructura para parametrizar los tests de salto
struct JumpTestCase {
    Opcode opcode;        // JMP
    CPU::AddressingMode mode;
    uint16_t baseAddr;    // dirección escrita en memoria para la instrucción
    uint16_t expectedPC;  // PC esperado después de ejecutar
    const char* name;
};

// Fixture para los tests de JMP
...
TEST_P(JumpInstructions, Execute) {
    ...
    // Escribir la instrucción y dirección en memoria según modo
    ...
    mem.write(0xFFFC, static_cast<uint8_t>(test.opcode));
    mem.write(0xFFFD, static_cast<uint8_t>(test.baseAddr & 0xFF));   // puntero low
    mem.write(0xFFFE, static_cast<uint8_t>((test.baseAddr >> 8) & 0xFF)); // puntero high

    uint16_t ptr = test.baseAddr; 
    uint16_t target = test.expectedPC;

    // Caso bug del 6502 si ptr termina en $FF
    uint16_t highAddr = ((ptr & 0x00FF) == 0xFF) ? (ptr & 0xFF00) : ptr + 1;

    mem.write(ptr, static_cast<uint8_t>(target & 0xFF));           // target low
    mem.write(highAddr, static_cast<uint8_t>((target >> 8) & 0xFF)); // target high
}
