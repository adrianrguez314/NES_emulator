...
TEST(TransferInstructions, Transfers) {
    ...
    mem.write(0x0002, static_cast<uint8_t>(Ops::TAX));
    cpu.executeInstruction();
    EXPECT_EQ(cpu.getRegister('X'), 0x40);

    mem.write(0x0003, static_cast<uint8_t>(Ops::TAY));
    cpu.executeInstruction();
    ...
    mem.write(0x0004, static_cast<uint8_t>(Ops::TXA));
    cpu.executeInstruction();
    EXPECT_EQ(cpu.getRegister('A'), 0x80);

    mem.write(0x0005, static_cast<uint8_t>(Ops::TYA));
    cpu.executeInstruction();
